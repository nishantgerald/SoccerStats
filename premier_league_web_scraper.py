...
def fbref_get_epl_standings():
    ...
    for team in SOUP.find("table", id=re.compile("^results.*overall$")).select(
        "[data-stat='rank']"
    ):
...
def fbref_get_squad_standard_stats():
    ...
    for team in SOUP.find("table", id="stats_squads_standard_for").select(
        "[data-stat='team']"
    ):
        teams_list.append(team.get_text())
    for team in SOUP.find("table", id="stats_squads_standard_for").select(
        "[data-stat='players_used']"
    ):
    ...
    get_squad_standard_stats_dict[
        "no_players_used_in_game_list"
    ] = no_players_used_in_game_list[1:]
...
def fbref_get_squad_goalkeeping_stats():
    ...
    for team in SOUP.find("table", id="stats_squads_keeper_for").select(
        "[data-stat='team']"
    ):
        teams_list.append(team.get_text())
    for team in SOUP.find("table", id="stats_squads_keeper_for").select(
        "[data-stat='gk_goals_against']"
    ):
        gk_goals_against_list.append(team.get_text())
    for team in SOUP.find("table", id="stats_squads_keeper_for").select(
        "[data-stat='gk_goals_against_per90']"
    ):
        gk_goals_against_per90_list.append(team.get_text())
    for team in SOUP.find("table", id="stats_squads_keeper_for").select(
        "[data-stat='gk_shots_on_target_against']"
    ):
    ...
    get_squad_goalkeeping_stats_dict["gk_pens_save_pct_list"] = gk_pens_save_pct_list[
        1:
    ]
    get_squad_goalkeeping_stats_df = pd.DataFrame(
        get_squad_goalkeeping_stats_dict
    ).rename(
        columns={
            "teams_list": "CLUB",
...
def fbref_get_squad_shooting_stats():
    ...
    for team in SOUP.find("table", id="stats_squads_shooting_for").select(
        "[data-stat='team']"
    ):
        teams_list.append(team.get_text())
    for team in SOUP.find("table", id="stats_squads_shooting_for").select(
